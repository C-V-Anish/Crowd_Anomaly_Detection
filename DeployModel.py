...
def WriteInfo(err, text, norm_count, anom_count):
    mp.PrintInline(
        "{4}, Frame Status: {0}, Normal Frame Count: {1}/{2}, Anomaly Frame Count {3}/{2}".format(
...
def RealTimeDetection(
    model, threshold, serve_type="real-time", vid_path=None, verbose=True
):
    if serve_type == "real-time":
        ...
        if vid_path == None:
            raise TypeError(
                "Value of `vid_path` argument cannot be `None`, when `serve_type` value is `video`. Provide valid path of `str` datatype."
            )
    ...
    while cap.isOpened():
        ...
        if ret == True:
            ...
            test_history["Normal Frames"].append(norm_count)
...
def StaticServing(
    path, model, threshold, frames_ext, serve_type="frames", verbose=True
):
...
def DeploySystem(
    ...
    if preset_threshold:
        if threshold is not None:
            raise TypeError(
                "Invalid value given to argument `threshold`, its value must be None when `preset_threshold` argument is set to True."
            )
        if data_model == "UCSD":
            threshold = 0.00026
            ...
            raise ValueError(
                "Invalid value given to the Argument `data_model`, it can be either `UCSD` or `Avenue`!"
            )
    else:
        if threshold is None:
            raise TypeError(
                "None value given to argument `threshold`, it cannot be None when `preset_threshold` argument is set to False, provide a value of `float` datype or set the `preset_threshold` argument to True, to use Preset Values of Threshold."
            )
    if serve_type != "real-time" and serve_type != None:
        if path is None:
            raise TypeError(
                "None value given to argument `path`, it cannot be None when value of `serve_type` is other than None."
            )
    if config_gpu:
    ...
    # should have the frames.
        ...
        test_hist = StaticServing(
            path, model, threshold, frames_ext, serve_type, verbose=verbose
        )
    ##Serve the Anomaly Detection from the .npy file.
        ...
        raise ValueError(
            "Invalid value given to the  `serve_type` argument. Possible values: {0}".format(
                serving_types
            )
        )
    return test_hist
