...
def TF_GPUsetup(GB=4):
    ...
    if type(GB) != int:
        raise TypeError(
            "Type of Parameter `GB` must be `int` and it should be 1 GB less than your GPU memory"
        )
...
def GetTrainData(name, re_shape=(-1, 227, 227, 10)):
    if type(name) != str:
        raise TypeError(
            "Provide a valid name of `string` datatype, to the `.npy` file."
        )
    if ".npy" not in name:
        name += ".npy"
...
def ShowVideo(cap, v_frame, text):
    ...
    if cv2.waitKey(25) & 0xFF == ord("q"):
        ...
        raise KeyboardInterrupt(
            "Real Time Anomoly Detection Stopped due to Keyboard Interrupt!"
        )
